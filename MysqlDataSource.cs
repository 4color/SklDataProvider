...
namespace SklDataProvider
{
    ...
    /// </summary>
    /// 
    public class MysqlDataSource : SklDataSource
    {
        ...
        public override IDbDataParameter[] CreateParameters(string[] paramNames, object[] paramValues)
        {
            ...
            if (paramValues != null)
            {
                ...
                for (int index = 0; index < length; index++)
                    parameters[index] = new MySqlParameter(paramNames[index], paramValues[index]);
            }
            else
            {
                for (int index = 0; index < length; index++)
                {
                    parameters[index] = new MySqlParameter();
        ...
        public override void UpdateDataSet(IDbCommand insertCommand, IDbCommand deleteCommand, IDbCommand updateCommand, DataSet dataSet, string tableName)
        {
            ...
            using (MySqlDataAdapter dataAdapter = new MySqlDataAdapter())
            {
                ...
                dataAdapter.UpdateCommand = (MySqlCommand)updateCommand;
                dataAdapter.InsertCommand = (MySqlCommand)insertCommand;
                dataAdapter.DeleteCommand = (MySqlCommand)deleteCommand;
        ...
        public override void UpdateDataSet(DataSet dataSet, string tableName, string sql)
        {
            ...
            using (MySqlDataAdapter dataAdapter = new MySqlDataAdapter(sql, this._connection))
            {

                MySqlCommandBuilder oleDbComandBuilder = new MySqlCommandBuilder(dataAdapter);
        ...
        public override IDbDataParameter[] CreateNTextParameter(string paramenterContent, string varName)
        {
            ...
            p[0] = new MySqlParameter();
            p[0].MySqlDbType = MySqlDbType.VarChar;
        ...
        public override IDbDataParameter[] CreateParameterS(string[] paramenterContentS, string[] varNameS, string[] dbTypeS)
        {
            ...
            {
                MySqlParameter[] p = new MySqlParameter[paramenterContentS.Length];
                for (int i = 0; i < p.Length; i++)
                {
                    p[i] = new MySqlParameter();
                    p[i].MySqlDbType = DbUtility.GetMysqlDbType(dbTypeS[i]);
        ...
        public override IDbDataParameter[] CreateBlobParameter(byte[] paramenterContent, string varName)
        {
            ...
            MySqlParameter[] p = new MySqlParameter[1];
            p[0] = new MySqlParameter();
            p[0].MySqlDbType = MySqlDbType.VarBinary;
            p[0].Direction = ParameterDirection.Input;
            p[0].ParameterName = varName;
            p[0].Value = paramenterContent;
            return p;
